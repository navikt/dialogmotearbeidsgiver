...
const Brodsmuler = ({ brodsmuler }) => {
    return (<nav className="brodsmuler" aria-label="Du er her: ">
        <img src="/dialogmotearbeidsgiver/img/svg/person.svg" alt="Du" className="brodsmuler__ikon" />
        <div className="brodsmuler__smuler">
            {
                brodsmuler
                    .map((smule, index) => {
                        return <Brodsmule key={index} {...smule} enesteSmule={brodsmuler.length === 1} sisteSmule={brodsmuler.length === index + 1} />;
                    })
            }
        </div>
    </nav>);
